...
class reportGenerator {
  ...
  generateTrendingPostsReport(keywords = [], includeKeywords = true, userAttributes = {}) {
    ...
    const filteredPosts = this.filterPostsByKeyword(allPosts, keywords, includeKeywords);

    //filter posts based on user attributes
    const finalPosts = this.filterPostsByUserAttributes(filteredPosts, userAttributes);

    //sort posts by the number of views (trending rate)
    const sortedPosts = finalPosts.sort((a, b) => b.views.length - a.views.length);
...
const reportGen = new reportGenerator([user1, user2, user3]);
